...
const { NewAppointmentPage } = require('../pages/newAppointment.page');
...
    patientFullName: 'p01 mid 01', // Tên đầy đủ của bệnh nhân sau khi tìm kiếm
    
    // Dữ liệu phản hồi tạo (Create Response) mong đợi
    expectedCreateMessage: "cm13", 
    
    // Dữ liệu phản hồi xóa (Delete Response) mong đợi
    expectedDeleteMessage: "cm13", 
    
    // Lỗi mong đợi khi lấy tài nguyên đã bị xóa (đã thêm)
    expectedErrorMessageAfterDelete: "cm22", 
};

// --- Cấu hình API ---
const API_BASE_URL = 'https://crgback.rcd.vnclever.com/api/v1/graphql'; 

test.describe('Add Appointment', () => {

    test('@TC001 - Create appointment via UI and verify/delete via API', async ({ page, request }) => {
        
        // Khởi tạo Page Object
        const newAppointment = new NewAppointmentPage(page);

        // 0. Truy cập trang chính sau khi Global Setup đăng nhập
        await newAppointment.goto(); 
...
        // KHAI BÁO BIẾN DÙNG CHUNG BẰNG 'let' ĐỂ CÓ THỂ GÁN LẠI (Đã gỡ comment)
        let appointmentId;
        let apiResponse;
        let responseBody;
...
        await newAppointment.newAppointmentBtn.click();
        
        // Nhập từ khóa tìm kiếm
        await newAppointment.selectPatient(TEST_DATA.patientSearchTerm); 
        
...
        await newAppointment.saveButton.click(); 
...
        // Xử lý trường hợp API trả về lỗi GraphQL (data: null)
...
        }
        
        // Ưu tiên 'createAppointment' làm key chính xác
...
        
        // 2. Assert 'message'
        expect(createAppointmentResult.message, `Trường 'message' phải là ${TEST_DATA.expectedCreateMessage}`).toEqual(TEST_DATA.expectedCreateMessage);
        
        // 3. Trích xuất 'id'
...
        
        // Giả định: imgWorkListId là một mảng và chúng ta lấy bệnh nhân từ phần tử đầu tiên
...
        expect(apiResponse.status()).toBe(200);
        const responseBodyAfterDelete = await apiResponse.json();
        const errors = responseBodyAfterDelete?.errors;
        
        console.log('--- PHẢN HỒI API SAU KHI XÓA (GIAI ĐOẠN 4) ---');
        console.log(JSON.stringify(responseBodyAfterDelete, null, 2));
        console.log('------------------------------------------------');

        // ASSERT 1: data phải là NULL (hoặc không tìm thấy) VÀ errors phải tồn tại
        expect(responseBodyAfterDelete.data).toBeNull();
        expect(errors).toBeDefined();
        expect(errors.length).toBeGreaterThan(0);
        
        // ASSERT 2: Kiểm tra message lỗi (Truy cập vào errors[0].message)
        const firstErrorMessage = errors[0].message;
        expect(firstErrorMessage).toEqual(TEST_DATA.expectedErrorMessageAfterDelete); 
        
        console.log(`✅ API: Xác minh hoàn tất. Appointment không còn tồn tại và đã trả về lỗi: ${firstErrorMessage}`);
    });
});